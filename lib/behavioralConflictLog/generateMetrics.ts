import type { BehavioralMetrics } from '@/types/behavioralConflict'

export const generateBehavioralMetrics = (): BehavioralMetrics => {
  return {
    stability: Math.round((0.4 + Math.random() * 0.6) * 100) / 100,
    variability: Math.round((0.3 + Math.random() * 0.7) * 100) / 100,
    deviation: Math.round((0.2 + Math.random() * 0.5) * 100) / 100,
    reactivity: Math.round((0.5 + Math.random() * 0.5) * 100) / 100,
  }
}
